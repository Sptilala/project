  ...
  $file_name= $time.'-'.$imagetype;
     ...
     //  Check if file already exists
        ...
        if (move_uploaded_file($_FILES["image"]["tmp_name"], $target_file)) {
         ...
         $q ="UPDATE `daily_news` SET  `product_name`='".$product_name."',`news`='".$news."',`image`='".$file_name."'  WHERE `id` = '".$pageid."'";
         $result = $con->query($q);
         ...
         $q ="UPDATE `daily_news` SET  `product_name`='".$product_name."',`news`='".$news."' WHERE `id` = '".$pageid."'";
         $result = $con->query($q);
